# -*- coding: utf-8 -*-
"""
CDCF Generator for MS SQL MultiTable
Generates JSON instructions for MS SQL CDC -> Kafka pipeline
"""
...
def main(metadata_file: str, output_file: str = None):
    ...
    print(f"Source type: mssql")
    print(f"Tables found: {len(workflow.metaobjects) if hasattr(workflow, 'metaobjects') and workflow.metaobjects else 0}")

    # Prepare template context
    current_time = datetime.now()
    current_date = current_time.strftime("%Y-%m-%d %H:%M:%S.%f")[:23]  # Обрезаем до миллисекунд, убираем проблемные символы
    ...
    if output_file is None:
        ...
        output_file = os.path.join(GENERATED_PATH, f"{workflow_name}_mssql_cdcf.json")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate CDCF JSON for MS SQL MultiTable")
    parser.add_argument('--input', '-i', required=True, help='Path to metamodel JSON file')
    parser.add_argument('--output', '-o', help='Output JSON file path (default: generated/<workflow>_mssql_cdcf.json)')
