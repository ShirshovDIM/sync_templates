# -*- coding: utf-8 -*-
"""
CDCF Generator for PostgreSQL MultiTable
Generates JSON instructions for PostgreSQL CDC -> Kafka pipeline
"""
...
def validate_metamodel(workflow: Any) -> List[str]:
    ...
    required_fields = [
        ...
        "propvalues_json.cdcf.debezium.publication_name",
        "propvalues_json.cdcf.debezium.slot_name",
    ]
...
def main(metadata_file: str, output_file: str = None):
    ...
    # Load template
    template_name = "cdcf_postgres.j2"
    try:
    ...
    try:
        ...
        # Check for Jinja raise statements
    ...
    if output_file is None:
        ...
        output_file = os.path.join(GENERATED_PATH, f"{workflow_name}_postgres_cdcf.json")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate CDCF JSON for PostgreSQL MultiTable")
    parser.add_argument('--input', '-i', required=True, help='Path to metamodel JSON file')
    parser.add_argument('--output', '-o', help='Output JSON file path (default: generated/<workflow>_postgres_cdcf.json)')
