# -*- coding: utf-8 -*-
"""
CDCF Generator for Oracle MultiTable
Generates JSON instructions for Oracle CDC -> Kafka pipeline
"""
...
class MissingRequiredFieldError(Exception):
    ...
    def __init__(self, missing_fields: List[str]):
        ...
        msg = "Missing required fields: " + ', '.join(missing_fields) + "\n\n" + \
              ...
              "    database_adapter: 'xstream'\n" + \
              "    out_server_name: '<name>'\n" + \
              "    database_pdb_name: '<PDB_NAME>'\n" + \
              "  }\n" + \
              "}"
        super().__init__(msg)
...
def validate_metamodel(workflow: Any) -> List[str]:
    ...
    required_fields = [
        ...
        "propvalues_json.cdcf.debezium.database_adapter",
        "propvalues_json.cdcf.debezium.out_server_name",
        "propvalues_json.cdcf.debezium.database_pdb_name",
    ]
...
def main(metadata_file: str, output_file: str = None):
    ...
    if output_file is None:
        ...
        output_file = os.path.join(GENERATED_PATH, f"{workflow_name}_oracle_cdcf.json")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate CDCF JSON for Oracle MultiTable")
    parser.add_argument('--input', '-i', required=True, help='Path to metamodel JSON file')
    parser.add_argument('--output', '-o', help='Output JSON file path (default: generated/<workflow>_oracle_cdcf.json)')
